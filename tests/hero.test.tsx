...
describe("hero", () => {
  ...
  it("HeroTitle splits words into aria-hidden spans and labels the wrapper", () => {
    const text = "Building tools with AI.";
    ...
    expect(container.querySelector("h1")!.getAttribute("aria-label")).toBe(text);
  });

  it("Hero renders h1, two CTAs and the version badge", () => {
    render(<Hero locale="en" />);
    const h1 = screen.getByRole("heading", { level: 1 });
    expect(h1.getAttribute("aria-label")).toBe(en.hero.title);
